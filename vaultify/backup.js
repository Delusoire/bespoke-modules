...
export const getLibrary = async ()=>{
    const { items } = await LibraryAPI.getContents({
        limit: 50000,
        sortOrder: 1
    });
    const lib = {};
    ...
    const track = await LibraryAPI.getTracks({
        limit: 50000,
        sort: {
            field: "ADDED_AT",
            order: "ASC"
    ...
    const episode = await LibraryAPI.getEpisodes({
        limit: 50000,
        sort: {
            field: "ADDED_AT",
            order: "ASC"
...
export const getLocalStoreAPI = ()=>{
    return Object.entries(LocalStorageAPI.items).filter(([key])=>key.startsWith(LocalStorageAPI.namespace)).map(([key, value])=>[
            key.split(":")[1],
            value
        ]);
};
