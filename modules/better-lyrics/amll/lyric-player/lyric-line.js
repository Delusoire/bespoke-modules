import { createMatrix4, matrix4ToCSS, scaleMatrix4 } from "../utils/matrix.js";
...
// 	];
// }
function generateFadeGradient(width, padding = 0, bright = "rgba(0,0,0,0.85)", dark = "rgba(0,0,0,0.3)") {
    const totalAspect = 2 + width + padding;
    const widthInTotal = width / totalAspect;
    const leftPos = (1 - widthInTotal) / 2;
    return [
...
export function shouldEmphasize(word) {
    return word.endTime - word.startTime >= 1000 && word.word.trim().length <= 7 && word.word.trim().length > 1;
}
...
export class LyricLineEl extends EventTarget {
    lyricPlayer;
    lyricLine;
    lyricAdvanceDynamicLyricTime;
    element;
    left;
    top;
    scale;
    ...
    constructor(lyricPlayer, lyricLine = {
        ...
        this.lyricAdvanceDynamicLyricTime = true;
    ...
    setLine(line) {
        ...
        if (this.lyricLine.isDuet) {
            ...
            this.element.classList.remove(this.lyricPlayer.style.classes.lyricDuetLine);
        }
        this.rebuildElement();
        this.rebuildStyle();
    }
    ...
    rebuildStyle() {
        if (this._hide) {
            if (this.lastStyle !== "display:none;transform:translate(0,-10000px);") {
                ...
                this.element.setAttribute("style", "display:none;transform:translate(0,-10000px);");
            }
            return;
        }
        let style = "";
        // if (this.lyricPlayer.getEnableSpring()) {
        style += `transform:translate(${this.lineTransforms.posX.getCurrentPosition().toFixed(1)}px,${this.lineTransforms.posY.getCurrentPosition().toFixed(1)}px) scale(${this.lineTransforms.scale.getCurrentPosition().toFixed(4)});`;
    ...
    rebuildElement() {
        ...
        for (const chunk of chunkedWords){
            if (Array.isArray(chunk)) {
                ...
                for (const word of chunk){
                    ...
                    if (shouldEmphasize(merged)) {
                        ...
                        const realWord = {
                            ...
                            subElements: charEls,
                            elementAnimations: [
                                this.initFloatAnimation(merged, mainWordEl)
                            ],
                            maskAnimations: [],
                            width: 0,
                            height: 0,
                ...
                if (shouldEmphasize(merged)) {
                    this.splittedWords[this.splittedWords.length - 1].elementAnimations.push(...this.initEmphasizeAnimation(merged, characterElements, merged.endTime - merged.startTime, merged.startTime - this.lyricLine.startTime));
                }
                ...
                if (shouldEmphasize(chunk)) {
                    ...
                    realWord.elementAnimations.push(...this.initEmphasizeAnimation(chunk, charEls, duration, realWord.startTime - this.lyricLine.startTime));
    ...
    initEmphasizeAnimation(word, characterElements, duration, delay) {
        ...
        result = characterElements.flatMap((el, i, arr)=>{
            ...
            const frames = new Array(ANIMATION_FRAME_QUANTITY).fill(0).map((_, j)=>{
                ...
                return {
                    offset: x,
                    transform: `${matrix4ToCSS(mat, 4)} translate(${-transX * 0.05 * amount * ((arr.length - i) / arr.length) ** 2}em, ${-transX * 0.03 * amount}em)`,
                    textShadow: `0 0 ${Math.min(0.3, blur * 0.2)}em rgba(255, 255, 255, ${glowLevel})`
                };
            });
    ...
    get totalDuration() {
        return this.lyricLine.endTime + (this.lyricAdvanceDynamicLyricTime ? 400 : 0) - this.lyricLine.startTime;
    }
    updateMaskImage() {
        if (this._hide) {
            if (this._prevParentEl) {
    ...
    setTransform(left = this.left, top = this.top, scale = this.scale, opacity = 1, blur = 0, force = false, delay = 0, currentAbove = true) {
        ...
        if (force || !enableSpring) {
            ...
            if (force) {
                this.element.classList.add(this.lyricPlayer.style.classes.tmpDisableTransition);
            }
            ...
            if (force) {
                requestAnimationFrame(()=>{
                    this.element.classList.remove(this.lyricPlayer.style.classes.tmpDisableTransition);
                });
            }
        } else {
