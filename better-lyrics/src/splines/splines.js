...
import { matrixMultMatrix, remapScalar, scalarAddVector, scalarMultVector, vectorAddVector, vectorDist, vectorDivScalar, vectorMultVector, vectorSubVector } from "/modules/Delusoire/delulib/lib/math.js";
var EndCondition;
...
class Monomial {
    ...
    at(t, n = 0) {
        ...
        const weights = vectorDivScalar(_.range(n).map((i)=>scalarAddVector(i + 1, powers)).reduce((u, v)=>u.map((_, i)=>u[i] * v[i]), new Array(powers.length).fill(1)), (t1 - t0) ** n);
        const tps = powers.map((power)=>t ** power);
        return matrixMultMatrix([
            vectorMultVector(tps, weights)
        ], coefficients)[0];
    }
}
    ...
    constructor(vertices, tangents, grid = _.range(vertices.length)){
        ...
        const segments = _.zip(zip_vertices, zip_grid).map(([[x0, x1], [t0, t1]], i)=>{
            ...
            const control_values = [
                x0,
                x1,
                scalarMultVector(t1 - t0, v0),
                scalarMultVector(t1 - t0, v1)
            ];
...
export class KochanekBartels extends CubicHermite {
    ...
    static fromAlpha(vertices, tcb, alpha = 0, endconditions = [
        0,
        0
    ]) {
        const deltas = zip_n_uplets(2)(vertices).map(([x0, x1])=>vectorDist(x0, x1) ** alpha);
        const grid = deltas.reduce((partialSums, delta)=>[
                ...partialSums,
                partialSums.at(-1) + delta
            ], [
            0
        ]);
        return KochanekBartels.fromGrid(vertices, tcb, grid, endconditions);
    }
    static fromGrid(vertices, tcb, grid, endconditions = [
        0,
        0
    ]) {
        const closed = endconditions === 1;
...
function _end_tangent(condition, vertices, times, other_tangent) {
    return condition === 0 ? _natural_tangent(vertices, times, other_tangent) : condition;
}
