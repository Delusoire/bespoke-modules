...
var _dec,
	...
	_init_state,
	_init_container,
	_init_loadedLyricsType,
	_init_scrollTimeout,
	_init_scrollContainer,
	_initProto1;
...
let _AnimatedText;
(_dec = customElement(_AnimatedText.NAME)),
	(_dec1 = property({
		type: Boolean,
	}));
new (class extends _identity {
	...
	static {
		class AnimatedText extends (_AnimatedMixin = AnimatedMixin(SyncedMixin(LitElement))) {
			static {
				({
					e: [_init_split, _initProto],
					c: [_AnimatedText, _initClass],
				} = _apply_decs_2203_r(this, [[_dec1, 1, "split"]], [_dec], _AnimatedMixin));
			}
...
new (class extends _identity {
	...
	static {
		class DetailTimelineProvider extends (_SyncedContainerMixin = SyncedContainerMixin(SyncedMixin(LitElement))) {
			static {
				({
					c: [_DetailTimelineProvider, _initClass1],
				} = _apply_decs_2203_r(this, [], [_dec2], _SyncedContainerMixin));
			}
...
new (class extends _identity {
	...
	static {
		class TimelineProvider extends (_ScrolledMixin = ScrolledMixin(SyncedContainerMixin(SyncedMixin(LitElement)))) {
			static {
				({
					c: [_TimelineProvider, _initClass2],
				} = _apply_decs_2203_r(this, [], [_dec3], _ScrolledMixin));
			}
...
new (class extends _identity {
	...
	static {
		class LyricsContainer extends (_SyncedContainerMixin1 = SyncedContainerMixin(SyncedMixin(LitElement))) {
			static {
				({
					c: [_LyricsContainer, _initClass3],
				} = _apply_decs_2203_r(this, [], [_dec4], _SyncedContainerMixin1));
			}
...
let _LyricsWrapper;
(_dec5 = customElement(_LyricsWrapper.NAME)),
	(_dec6 = property({
		attribute: false,
	})),
	(_dec7 = provide({
	...
	(_dec8 = state()),
	(_dec9 = query(_LyricsContainer.NAME)),
	(_dec10 = provide({
...
new (class extends _identity {
	...
	static {
		class LyricsWrapper extends (_LitElement = LitElement) {
			static {
				({
					e: [_init_state, _init_container, _init_loadedLyricsType, _init_scrollTimeout, _init_scrollContainer, _initProto1],
					c: [_LyricsWrapper, _initClass4],
				} = _apply_decs_2203_r(
					this,
					[
						[_dec6, 1, "state"],
						[_dec9, 1, "container"],
						[[_dec7, _dec8], 0, "loadedLyricsType"],
						[_dec10, 0, "scrollTimeout"],
						[_dec11, 0, "scrollContainer"],
					],
					[_dec5],
					_LitElement,
				));
                    ...
                    <lyrics-container>
                        ${when(
							...
							() =>
								html`${map(
									lyrics.content,
									l => html`<timeline-provider tsp=${l.tsp} tep=${l.tep}
                                            >${map(
												l.content,
												w => html`<detail-timeline-provider tsp=${w.tsp} tep=${w.tep}
                                                        >${map(
															w.content.split(""),
															c => html`<animated-text
                                                                    tsp=${w.tsp}
                                                                    content=${c === " " ? " " : c}
                                                                ></animated-text>`,
...
export {
	_AnimatedText as AnimatedText,
	_DetailTimelineProvider as DetailTimelineProvider,
	_TimelineProvider as TimelineProvider,
	_LyricsContainer as LyricsContainer,
	_LyricsWrapper as LyricsWrapper,
};
