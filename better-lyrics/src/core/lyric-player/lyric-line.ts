import { LyricPlayer } from "./index.ts";
...
import { WebAnimationSpring } from "../utils/wa-spring.ts";
...
const norNum = (min: number, max: number) => (x: number) => Math.min(1, Math.max(0, (x - min) / (max - min)));
...
type MouseEventMap = {
	[evt in keyof HTMLElementEventMap]: HTMLElementEventMap[evt] extends MouseEvent ? evt
		: never;
};
...
export class LyricLineEl extends EventTarget implements HasElement, Disposable {
	...
	rebuildStyle() {
		...
		style += `transform:translate(${
			this.lineTransforms.posX
				.getCurrentPosition()
				.toFixed(1)
		}px,${
			this.lineTransforms.posY
				.getCurrentPosition()
				.toFixed(1)
		}px) scale(${
			this.lineTransforms.scale
				.getCurrentPosition()
				.toFixed(4)
		});`;
	...
	private generateWebAnimationBasedMaskImage() {
		...
		this.splittedWords.forEach((word, i) => {
			...
			if (wordEl) {
				...
				const widthBeforeSelf = this.splittedWords.slice(0, i).reduce((a, b) => a + b.width, 0) +
