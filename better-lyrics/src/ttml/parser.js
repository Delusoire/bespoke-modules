const timeRegexp = /^(((?<hour>[0-9]+):)?(?<min>[0-9]+):)?(?<sec>[0-9]+([.:]([0-9]+))?)/;
...
export function parseTTML(ttmlText) {
    ...
    const lyricLines = [];
    function parseParseLine(lineEl, isBG = false, isDuet = false) {
        const line = {
            words: [],
            translatedLyric: "",
            romanLyric: "",
            isBG,
            isDuet: !!lineEl.getAttribute("ttm:agent") && lineEl.getAttribute("ttm:agent") !== mainAgentId,
            startTime: 0,
            endTime: 0
        };
        ...
        if (startTime && endTime) {
            ...
            line.startTime = line.words.filter((v)=>v.word.trim().length > 0).reduce((pv, cv)=>Math.min(pv, cv.startTime), Infinity);
            line.endTime = line.words.filter((v)=>v.word.trim().length > 0).reduce((pv, cv)=>Math.max(pv, cv.endTime), 0);
        }
        if (haveBg) {
