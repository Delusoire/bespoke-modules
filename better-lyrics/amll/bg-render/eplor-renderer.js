...
const vertShader = await fetch("/modules/Delusoire/better-lyrics/amll/bg-render/shaders/base.vert.glsl").then((
	res,
) => res.text());
const fragShader = await fetch("/modules/Delusoire/better-lyrics/amll/bg-render/shaders/base.frag.glsl").then((
	res,
) => res.text());
const blendShader = await fetch("/modules/Delusoire/better-lyrics/amll/bg-render/shaders/blend.frag.glsl").then(
	(res) => res.text(),
);
const eplorShader = await fetch("/modules/Delusoire/better-lyrics/amll/bg-render/shaders/eplor.frag.glsl").then(
	(res) => res.text(),
);
const noiseShader = await fetch("/modules/Delusoire/better-lyrics/amll/bg-render/shaders/noise.frag.glsl").then(
	(res) => res.text(),
);
...
export class EplorRenderer extends BaseRenderer {
	...
	onRedraw(tickTime, delta) {
		...
		this.mainProgram.setUniform2f("f", this.renderSize[0], this.renderSize[1]);
		this.mainProgram.setUniform1f("b", tickTime / 1000);
		this.mainProgram.setUniform1f("a", this.hasLyricValue);
		this.mainProgram.setUniform1f("c", this.hasLyric ? this._lowFreqVolume : 0.0);
		...
		this.mainProgram.setUniform2f("g", this.g[0], this.g[1]);
		this.mainProgram.setUniform1f("d", window.innerWidth > 1024 ? 1 : 0);
