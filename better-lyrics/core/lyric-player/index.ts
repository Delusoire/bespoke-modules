...
import type { Disposable, HasElement, LyricLine } from "../interfaces.ts";
import { eqSet } from "../utils/eq-set.ts";
import { SpringParams } from "../utils/spring.ts";
import { BottomLineEl } from "./bottom-line.ts";
import { InterludeDots } from "./interlude-dots.ts";
import { LyricLineEl, RawLyricLineMouseEvent, shouldEmphasize } from "./lyric-line.ts";
...
export class LyricPlayer extends EventTarget implements HasElement, Disposable {
	...
	private lyricLinesSize: WeakMap<LyricLineEl, [number, number]> = new WeakMap();
	...
	private resizeObserver: ResizeObserver = new ResizeObserver((e) => {
		...
		const innerWidth = this.element.clientWidth -
			parseFloat(styles.paddingLeft) -
			parseFloat(styles.paddingRight);
		const innerHeight = this.element.clientHeight -
	...
	setLyricLines(lines: LyricLine[], initialTime = 0) {
		...
		this.processedLines = lines
			.filter(
				(line) => line.words.reduce((pv, cv) => pv + cv.word.trim().length, 0) > 0,
			)
			.map((line, i, lines) => {
				if (line.isBG) {
					return {
				...
				if (lastLine?.isBG && pastLine) {
					if (pastLine.endTime < line.startTime) {
						return {
							...line,
							startTime: Math.max(pastLine.endTime, line.startTime - timeOffset) ||
								line.startTime,
						};
					}
					...
					if (lastLine.endTime < line.startTime) {
						return {
							...line,
							startTime: Math.max(lastLine?.endTime, line.startTime - timeOffset) ||
								line.startTime,
						};
					}
				}
	...
	calcLayout(force = false, reflow = false) {
		...
		const scrollOffset = this.lyricLinesEl
			...
			.reduce(
				(acc, el) => acc + (el.getLine().isBG ? 0 : this.lyricLinesSize.get(el)?.[1] ?? 0),
				0,
			);
