 ...
 * 一个播放歌词的组件
 * @author SteveXMH
 */ import { eqSet } from "../utils/eq-set.js";
import { BottomLineEl } from "./bottom-line.js";
import { InterludeDots } from "./interlude-dots.js";
import { LyricLineEl, shouldEmphasize } from "./lyric-line.js";
import { create } from "https://esm.sh/jss";
import preset from "https://esm.sh/jss-preset-default";
 ...
 */ export class LyricPlayer extends EventTarget {
    ...
    processedLines = [];
    lyricLinesEl = [];
    lyricLinesSize = new WeakMap();
    lyricLinesIndexes = new WeakMap();
    hotLines = new Set();
    ...
    isScrolled = false;
    isSeeking = false;
    initializeSeeking = false;
    invokedByScrollEvent = false;
    scrollOffset = 0;
    ...
    resizeObserver = new ResizeObserver((e)=>{
        ...
        const innerWidth1 = this.element.clientWidth - parseFloat(styles.paddingLeft) - parseFloat(styles.paddingRight);
        const innerHeight1 = this.element.clientHeight - parseFloat(styles.paddingTop) - parseFloat(styles.paddingBottom);
        this.innerSize[0] = innerWidth1;
        this.innerSize[1] = innerHeight1;
        this.rebuildStyle();
    ...
    _baseFontSize = parseFloat(getComputedStyle(this.element).fontSize);
    get baseFontSize() {
    ...
    constructor(){
        ...
        this.element.addEventListener("touchmove", (evt)=>{
            if (this.beginScrollHandler()) {
                ...
                const targetDirection = lastDelta > 0 ? "down" : lastDelta < 0 ? "up" : "none";
        ...
        this.element.addEventListener("touchend", (evt)=>{
            if (this.beginScrollHandler()) {
                ...
                scrollSpeed = (this.scrollOffset - startScrollY) / scrollDuration * 1000;
                let lt = 0;
                const curScrollId = Symbol("amll-scroll");
                scrollId = curScrollId;
                const onScrollFrame = (dt)=>{
                    lt ||= dt;
                    if (scrollId === curScrollId && this.beginScrollHandler()) {
                        ...
                        this.limitScrollOffset();
                        this.calcLayout(true);
                        if (Math.abs(scrollSpeed) > 1 && !this.scrollBoundary.includes(this.scrollOffset)) {
	 ...
	 */ getCurrentInterlude() {
        ...
        } else if (this.processedLines[i]?.endTime && this.processedLines[i + 1]?.startTime) {
            if (this.processedLines[i + 1].startTime > currentTime && this.processedLines[i].endTime < currentTime) {
                return [
	 ...
	 */ rebuildStyle() {
        ...
        if (window.innerWidth <= 1024) {
            ...
            style += `${this.innerSize[0] - this.padding * (this.isNonDuet ? 1.5 : 4)}px;`;
        }
	 ...
	 */ setLyricLines(lines, initialTime = 0) {
        ...
        this.processedLines = lines.filter((line)=>line.words.reduce((pv, cv)=>pv + cv.word.trim().length, 0) > 0).map((line, i, lines)=>{
            if (line.isBG) {
                return {
                    ...line
                };
            ...
            if (lastLine?.isBG && pastLine) {
                if (pastLine.endTime < line.startTime) {
                    return {
                        ...line,
                        startTime: Math.max(pastLine.endTime, line.startTime - timeOffset) || line.startTime
                    };
                }
                ...
                if (lastLine.endTime < line.startTime) {
                    return {
                        ...line,
                        startTime: Math.max(lastLine?.endTime, line.startTime - timeOffset) || line.startTime
                    };
                }
            }
            return {
	 ...
	 */ calcLayout(force = false, reflow = false) {
        ...
        const scrollOffset = this.lyricLinesEl.slice(0, targetAlignIndex).reduce((acc, el)=>acc + (el.getLine().isBG ? 0 : this.lyricLinesSize.get(el)?.[1] ?? 0), 0);
        ...
        this.lyricLinesEl.forEach((el, i)=>{
            ...
            if (!setDots && interludeDuration >= 5000 && (i === this.scrollToIndex && interlude?.[2] === -2 || i === this.scrollToIndex + 1)) {
                setDots = true;
            ...
            el.setTransform(left, curPos, isActive ? 1 : line.isBG ? 0.75 : SCALE_ASPECT, targetOpacity, window.innerWidth <= 1024 ? blurLevel * 0.8 : blurLevel, force, delay, i < (interlude ? interlude[2] + 1 : this.scrollToIndex));
	 ...
	 */ setCurrentTime(time, isSeek = false) {
        ...
        if (isSeek) {
            if (this.bufferedLines.size > 0) {
                ...
                this.scrollToIndex = this.processedLines.findIndex((line)=>line.startTime >= time);
            }
            this.bufferedLines.clear();
    ...
    setLyricAdvanceDynamicLyricTime(enable) {
        for (const line of this.lyricLinesEl){
	 ...
	 */ setLinePosXSpringParams(params) {
