...
import { Cosmos, Platform } from "/modules/official/stdlib/src/expose/Platform.js";
...
export const setInLibrary = (uris, liked)=>LibraryAPI[liked ? "add" : "remove"]({
        uris
    });
...
export const fetchLikedTracks = async ()=>(await LibraryAPI.getTracks({
        limit: Number.MAX_SAFE_INTEGER
    })).items;
export const fetchArtistLikedTracks = async (uri, offset = 0, limit = 100)=>(await LibraryAPI.getTracks({
        uri,
        offset,
        limit
    })).items;
...
export const addPlaylist = async (playlist, folder)=>await RootlistAPI.add([
        playlist
    ], folder ? SpotifyLoc.after.fromUri(folder) : {});
/* Replaced by createPlaylistFromTracks */ export const createPlaylist = async (name, location = {})=>await RootlistAPI.createPlaylist(name, location);
export const createPlaylistFromTracks = (name, tracks, folder)=>Cosmos.post("sp://core-playlist/v1/rootlist?responseFormat=protobufJson", {
        operation: "create",
        ...folder ? {
            after: folder
        } : {},
...
export const setPlaylistVisibility = async (playlist, visibleForAll)=>await PlaylistPermissionsAPI.setBasePermission(playlist, visibleForAll ? "VIEWER" : "BLOCKED");
export const setPlaylistPublished = async (playlist, published)=>await RootlistAPI.setPublishedState(playlist, published);
export const fetchFolder = async (folder)=>await RootlistAPI.getContents({
        folderUri: folder
    });
...
export const movePlaylistTracks = async (playlist, uids, location = {})=>await PlaylistAPI.move(playlist, uids.map((uid)=>({
            uid
        })), location);
