...
export const createPlaylistFromLastSortedQueue = async () => {
	if (globalThis.lastSortedQueue.length === 0) {
		S.Snackbar.enqueueSnackbar(ERROR.LAST_SORTED_QUEUE_EMPTY, {
			variant: "error",
		});
		return;
	}
	...
	if (!success) {
		S.Snackbar.enqueueSnackbar(`Failed to create Playlist ${playlistName}`, {
			variant: "error",
		});
		return;
	}
	setPlaylistVisibility(playlistUri, false);
	S.Snackbar.enqueueSnackbar(`Playlist ${playlistName} created`, {
		variant: "default",
	});
};
export const reordedPlaylistLikeSortedQueue = async () => {
	...
	if (!URI.is.PlaylistV1OrV2(lastFetchedUri)) {
		S.Snackbar.enqueueSnackbar(ERROR.LAST_SORTED_QUEUE_NOT_A_PLAYLIST, {
			variant: "error",
		});
		return;
	}
	...
	S.Snackbar.enqueueSnackbar("Reordered the sorted playlist", {
		variant: "default",
	});
	if (playlistUids.length) {
		S.Snackbar.enqueueSnackbar(`Left ${playlistUids.length} unordered at the bottom`, {
			variant: "default",
		});
	}
};
